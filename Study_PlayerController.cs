...
public class PlayerController : MonoBehaviour {   // MonoBehaviour: 이 코드를 유니티 캐릭터에 '부품(컴포넌트)'처럼 붙여서 쓸 수 있게 해주는 마법의 단어!
	...
	void Awake(){    //게임 시작 후에 제일 먼저 실행 되는것  게임이 실행되거나 오브젝트가 생성 될때 딱 한번만 실행 되는것 start()이거보다 더 먼저 실행됨  실행이 되는 이유는 필요한 도구들을 미리 챙기는 것임.
		//Get references to our components  // 내 몸에 붙은 부품을 찾는것. 이 코드는 유니티의 컴포넌트 기반 구조를 이해하는 핵심 유니티에서 캐릭터(오브젝트)는 레고 블럭과 같음 캐릭터라는 몸체에 Rigidbody2D(물리),CircleCollider2D(충돌범위) 같은 부품을 끼워넣는것임.
